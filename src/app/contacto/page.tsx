    ...
    const obtenerAtracciones = async () => {
      try {
        const respuesta = await axios.get<RespuestaAtracciones>(
          "https://francinyturismoproyect-api-5aa41d4e5161.herokuapp.com/api/route/Obtener_Las_Atracciones"
        );

        console.log("Respuesta atracciones:", respuesta.data);

        if (respuesta.data.codigoRespuesta === 0) {
          setListaAtraccionesPorProvincia(respuesta.data.detalle);
        } else {
          console.warn("La API devolvió un código inesperado:", respuesta.data);
        }
      } catch (error) {
        console.error("Error obteniendo atracciones:", error);
      }
    };
